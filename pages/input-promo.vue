<template>
    <div>
        <div class="flex justify-center">
            <div class="main-wrapper relative">
                <div class="flex w-full border-b border-gray-200 px-3 py-2 flex justify-center">
                    <img src="~/assets/logo.jpg" style="width:150px" />
                </div>
                <div class="p-3 border-b border-gray-300 pb-5">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <img style="width:30px"
                                src="data:image/png;base64,">
                            <div class="text-xl pl-3 font-bold">Bayar Paket</div>
                        </div>
                        <nuxt-link to="/"><img style="width:30px" src="data:image/png;base64,"></nuxt-link>
                    </div>
                    <div class="flex items-center mt-3">
                        <div class="flex border border-gray-200 bg-blue-50 p-2 w-full">
                            <img style="width:30px"
                                src="data:image/png;base64,">
                            <input type="text" class="w-full ml-2 bg-transparent" placeholder="Masukan kode kupon">

                        </div>
                        <button type="button"
                            class="pointer bg-primary text-white py-3 px-5 ml-3 rounded-md font-bold">Gunakan</button>
                    </div>
                </div>
                <div class="p-3">
                    <div class="flex items-center justify-between mb-2" v-for="a in 3">
                        <div class="flex items-center">
                            <img style="width:70px" src="https://png.pngtree.com/png-clipart/20220621/original/pngtree-food-delivery-pesanan-makanan-online-gojek-grab-png-transparent-background-vektor-png-image_8153435.png"/>
                            <div class="pl-3">
                                <div class="font-bold">Discount Rp 10.000</div>
                                <div class="text-gray-700">Minimum Spend Rp 100.000</div>
                            </div>
                        </div>
                        <div><img style="width:30px" src="data:image/png;base64,"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
<script setup lang="ts">
</script>